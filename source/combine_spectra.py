...
def load_spectra(file_path):
    ...
    if file_path.endswith('.mgf'):
        spectra = pyteomics.mgf.read(file_path)
    elif file_path.endswith('.mzML'):
        spectra = pyteomics.mzml.read(file_path)
    else:
        raise ValueError("Unsupported file format. Please provide .mgf or .mzML file.")
    
    return spectra

def combine_spectra(file_path =None, spectra_list=None, mzd=0.01, min_prop=0.5, weighted=False):
