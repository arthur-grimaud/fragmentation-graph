...
class SpectrumFile:
    ...
    def _load(self, file_path):
        ...
        if extension.lower() == ".mzml":
            ...
            self.get_by_id = self.get_by_id_MZML
            ...
            self.get_by_id = self.get_by_id_MGF

        else:
            self.logger.info(f"Cannot infer format from {file_path}, only mzML and MGF formats are supported")
            raise Exception("Unsupported spectra file format")

    def get_by_id_MGF(self, id_string):
    ...
    def get_by_id_MZML(self, id_string):
        if type(id_string) is int:
            return self.spectra_source.get_by_index(self.indices["scan"][id_string])
        elif type(id_string) is str:
            # in case of whitespaces
